...
if (isset($_GET['league_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $fixtures .= '<tr>
                            <td>' . $row['home_team'] . '<img width="30" src=admin/'.  getTeamById($row['home_id'])['team_logo'] . ' />' .  '</td>
                            <td>' . $row['away_team'] . '<img width="30" src=admin/'.  getTeamById($row['away_id'])['team_logo'] . ' />' .  '</td>
                            <td>' . $row['venue'] . '</td>
